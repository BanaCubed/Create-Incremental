<template>
  <div class="save" :class="{ active: isActive, readonly }">
    ...
    <div class="details" v-if="save.error == undefined && !isEditing">
      ...
      ><br />
      <div v-if="currentTime" class="time">Last played {{ dateFormat.format(currentTime) }}</div>
    </div>
...
const synced = computed(
  () =>
    !props.readonly && galaxy.value?.loggedIn === true && syncedSaves.value.includes(props.save.id),
);
