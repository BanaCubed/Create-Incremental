  ...
  <Modal v-model="isOpen" width="960px" ref="modal" :prevent-closing="true">
    ...
    <template v-slot:body>
      <div>
        ...
        {{ conflictingSaves.length > 1 ? "appear" : "appears" }} to be out of sync with your local
        {{ pluralizedSave }}. Which
        {{ pluralizedSave }}
        do you want to keep?
      </div>
      <br />
      ...
      >
        <div @click="selectCloud(i)" :class="{ selected: selectedSaves[i] === 'cloud' }">
          <h2>
            Cloud
            <span v-if="(conflict.cloud.time ?? 0) > (conflict.local.time ?? 0)" class="note"
              >(more recent)</span
            >
            <span
              v-if="(conflict.cloud.timePlayed ?? 0) > (conflict.local.timePlayed ?? 0)"
              class="note"
              >(more playtime)</span
            >
          </h2>
