  ...
  return createLazyProxy((feature) => {
    ...
    const requirement = {
      ...
      partialDisplay: (amount?: DecimalSource) => (
        <span
          style={Decimal.gt(unref(requirement.requirementMet), 0) ? "" : "color: var(--danger)"}
        >
          {displayResource(
            resource,
      ...
      requirementMet:
        cost instanceof Formula
          ? calculateMaxAffordable(cost, resource, cumulativeCost ?? true, directSum, maxBulkAmount)
          : computed(
              (): DecimalSource =>
                Decimal.gte(resource.value, unref(requirement.cost as MaybeRef<DecimalSource>))
                  ? 1
                  : 0,
            ),
      pay:
