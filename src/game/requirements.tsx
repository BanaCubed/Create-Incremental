...
export interface Requirement {
	/**
	 * The display for this specific requirement. This is used for displays multiple requirements
	 * condensed. Required if {@link visibility} can be {@link Visibility.Visible}.
	 */
	partialDisplay?: (amount?: DecimalSource) => Renderable;
	/**
	 * The display for this specific requirement. Required if {@link visibility} can be
	 * {@link Visibility.Visible}.
	 */
	display?: (amount?: DecimalSource) => Renderable;
	/**
	 * Whether or not this requirement should be displayed in Vue Features.
	 * {@link displayRequirements} will respect this property.
	 */
	...
	/**
	 * Whether or not this requirement will need to affect the game state when whatever is using
	 * this requirement gets triggered.
	 */
	requiresPay: MaybeRef<boolean>;
	/**
	 * Whether or not this requirement can have multiple levels of requirements that can be met at
	 * once. Requirement is assumed to not have multiple levels if this property not present.
	 */
...
export interface CostRequirementOptions {
	...
	resource: Resource;
	/**
	 * The amount of {@link resource} that must be met for this requirement. You can pass a formula,
	 * in which case maximizing will work out of the box (assuming its invertible and, for more
	 * accurate calculations, its integral is invertible). If you don't pass a formula then you can
	 * still support maximizing by passing a custom {@link pay} function.
	 */
	...
	/**
	 * Pass-through to {@link Requirement["requiresPay"]}. If not set to false, the default
	 * {@link pay} function will remove {@link cost} from {@link resource}.
	 */
	requiresPay?: MaybeRefOrGetter<boolean>;
	/**
	 * When calculating multiple levels to be handled at once, whether it should consider resources
	 * used for each level as spent. Setting this to false causes calculations to be faster with
	 * larger numbers and supports more math functions.
	 *
	 * @see {Formula}
	 */
	...
	/**
	 * When calculating requirement for multiple levels, how many should be directly summed for
	 * increase accuracy. High numbers can cause lag. Defaults to 10 if cumulative cost, 0
	 * otherwise.
	 */
...
/**
 * Lazily creates a requirement with the given options, that is based on meeting an amount of a
 * resource.
 *
...
/**
 * Calculates the maximum number of levels that could be acquired with the current requirement
 * states. True/false requirements will be counted as Infinity or 0.
 *
 * @param requirements The 1+ requirements to check
 */
