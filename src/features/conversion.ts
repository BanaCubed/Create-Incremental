...
export function createConversion<T extends ConversionOptions>(optionsFunc: () => T) {
  return createLazyProxy(() => {
    ...
    const currentGain =
      _currentGain == null
        ? computed((): Decimal => {
            let gain = Decimal.floor(conversion.formula.evaluate(baseResource.value)).max(0);
    ...
    const conversion = {
      ...
      currentAt:
        currentAt == null
          ? computed(
              (): DecimalSource =>
                conversion.formula.invert(Decimal.floor(unref(conversion.currentGain))),
            )
      ...
      nextAt:
        nextAt == null
          ? computed(
              (): DecimalSource =>
                conversion.formula.invert(Decimal.floor(unref(conversion.currentGain)).add(1)),
            )
