          ...
          <span>
            ...
            <b>
              {displayResource(
                ...
                Decimal.max(unref(conversion.actualGain), unref(resetButton.minimumGain)),
              )}
            </b>{" "}
            ...
            {unref(resetButton.showNextAt) != null ? (
              <div>
                ...
                {displayResource(
                  ...
                  !unref<boolean>(conversion.buyMax) && Decimal.gte(unref(conversion.actualGain), 1)
...
export function createCollapsibleModifierSections(
  ...
  const jsxFunc = () => {
    ...
    const sectionJSX = sections.map((s, i) => {
      ...
      const header = (
        <h3 onClick={() => (collapsed.value[i] = !collapsed.value[i])} style="cursor: pointer">
          <span class={"modifier-toggle" + (unref(collapsed.value[i]) ? " collapsed" : "")}>▼</span>
      ...
      return (
        <>
          ...
          <div
            style={{
              "--unit": settings.alignUnits && s.unit != null ? "'" + s.unit + "'" : "",
            }}
          >
            {header}
            <br />
  ...
  return computed(() => {
    if (unref(processedShowPreview)) {
      ...
      const preview = formatSmall(
        formula.evaluate(
          Decimal.add(unref(formula.innermostVariable ?? 0), unref(processedPreviewAmount)),
        ),
      );
      return (
        <>
